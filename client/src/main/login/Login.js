  ...
  return (
    <Switch>
      ...
      <Route exact path="/register">
        { !logInState.isLoggedIn ? <RegisterPage service={service} onLogin={handleLogin} /> : <Redirect to="/" /> }      
      </Route>
      <Route>
        { !logInState.isLoggedIn ? <Redirect to="/login" /> : <LoginContext.Provider value={{loginService: service}}>{children}</LoginContext.Provider> }
      </Route>
    </Switch>
  )
}