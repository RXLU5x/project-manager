 ...
 */
export default function LoginPage({ service, errorMessage, onLogin }) {
  ...
  async function handleSubmit(event) {
    ...
    if (isInputValid()) {
      ...
      let res = await userService.getUserID()
      if(res){
        service.setID(res)
        onLogin()
      }else{
    ...
    <div className='ui middle aligned center aligned grid' style={{ marginTop: 125 }}>
      <div className='column' style={{maxWidth: 380}}>
        <h2 className='ui header centered'>
          <div className='content'>Welcome to Projects Management UI</div>
        </h2>
